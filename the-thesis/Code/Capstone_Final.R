...
# Calculating a simple difference in outcomes before and after treatment (same method the government used to report the outcomes of the program) for the share of students who scored below the threshold 

astana_df <- subset(new_df, Region == "Astana City")

# Finding difference between 2018 and 2017
diff1 <- astana_df$Total.Low..City..[astana_df$Year == 2018] - astana_df$Total.Low..City..[astana_df$Year == 2017]
cat("Difference between years 2017 and 2018:", diff1, "\n")
# Applying the same methodology as the government, the effect is not as significant as they report, at least for those who score below the threshold, limiting their opportunities in the future

# Calculating Difference-in-Differences estimator for Karaganda

new_df$Post_Treatment <- ifelse(new_df$Year > 2018, 1, 0)

new_df <- new_df %>% 
  filter(Region %in% c("Astana City", "Karaganda"))

new_df$did <- new_df$Treatment*new_df$Post_Treatment
# Running a DID without controlling for other variables
did0 <- lm(Total.Low..City.. ~ Treatment + did,data = new_df)
summary(did0)
# The coefficient of 0.04 at p = 0.66 due to limited amount of data and the small post-treatment period
# Running a DID controlling for other variables
did <- lm(Total.Low..City.. ~ Treatment + did + Population_City + New_Schools + Number_of_Schools_City + Number_of_Students_City + Number_of_Teachers,data = new_df)
summary(did)
# A very small negative coefficient at p = 0.35 (I later linearly transform the confounding variables which decreases the model's sensitivity)

# Calculating Difference-in-Differences estimator for Aktobe
...
new_df <- new_df %>% 
  filter(Region %in% c("Astana City", "Aktobe"))

new_df$did <- new_df$Treatment*new_df$Post_Treatment
# Running a DID without controlling for other variables
did1 <- lm(Total.Low..City.. ~ Treatment + did,data = new_df)
summary(did1)
# The coefficient of 0.04 at p = 0.68 due to limited amount of data and the small post-treatment period
# Running a DID controlling for other variables
did2 <- lm(Total.Low..City.. ~ Treatment + did + Population_City + New_Schools + Number_of_Schools_City + Number_of_Students_City + Number_of_Teachers,data = new_df)
summary(did2)
# A very small negative coefficient at p = 0.9 (I later linearly transform the confounding variables which decreases the model's sensitivity)

############## Calculating a DID, SCM, and SDID estimators using synthdid package ############## 
...
# Plotting all three estimators together in one plot
...
# Constructing a synthetic control unit for each unit in the donor pool
placebo <- generate.placebos(dataprep.out = dataprep.out,
                             synth.out = synth.out, strategy = "multiprocess")

# Plotting the placebo tests
plot_placebos(placebo, discard.extreme=TRUE, xlab='Year' )
